...
class L2CacheManager:
    ...
    def should_spill(self, node_is_pinned: bool, ttl_expired: bool = True) -> bool:
        """
        ...
        Args:
            ...
            ttl_expired: Whether the TTL has expired (default True for unconditional)
        
        Returns:
            True if the node should be spilled to L2, False otherwise
        
        LMCache-style behavior:
        - write_back: TTL-protected nodes are NOT spilled (kept in L1)
        - write_through: Always spill
        """
        if self.write_policy == "write_back":
            # write_back: TTL-protected nodes stay in L1
            # Only spill if TTL has expired or node is not pinned
    ...
    def spill_to_l2(
        ...
        # Track L2 backup time in StateManager (for AIConfigurator integration)
        backup_time = token_count * 512 / (32 * 1e9)  # seconds
    ...
    def load_all_for_program(
        ...
        # Track L2 load time in StateManager (for AIConfigurator integration)
        if total_tokens > 0:
            # Calculate H2D time based on bytes per token and PCIe bandwidth
            # Default: 512 bytes/token, 32 GB/s PCIe
            h2d_time = total_tokens * 512 / (32 * 1e9)  # seconds
