...
# Try to import aiconfigurator, but make it optional
try:
    from sglang_simulator.time_predictor.aiconfigurator import get_perf_model
    HAS_AICONFIGURATOR = True
except ImportError:
    HAS_AICONFIGURATOR = False
    get_perf_model = None
...
def estimate_kv_cache_pool_capacity(
    ...
    """Estimate KV cache pool capacity.
    
    If aiconfigurator is available, uses it for accurate estimation.
    Otherwise, uses a simple heuristic based on available memory.
    """
    if HAS_AICONFIGURATOR and get_perf_model is not None:
        try:
            ...
            logger.warning(f"Failed to get weights from aiconfigurator: {e}")
            weights = 0
    else:
        weights = 0
